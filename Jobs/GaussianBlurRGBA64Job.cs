...
namespace RawTextureDataProcessingExamples
{
    ...
    public struct GaussianBlurRGBA64Job : Unity.Jobs.IJob
    {
        [DeallocateOnJobCompletion] NativeArray<RGBA64> copy;
        NativeArray<RGBA64> results;
        readonly int w, h;
        readonly float sigma;
        [DeallocateOnJobCompletion] NativeArray<float> boxes;
        public GaussianBlurRGBA64Job(NativeArray<RGBA64> data, int texture_width, int texture_height, float sigma)
        {
            this.results = data;
            this.copy = new NativeArray<RGBA64>(data, Allocator.TempJob);
        ...
        void BoxBlur(NativeArray<RGBA64> src, NativeArray<RGBA64> dst, int r)
        {
            BoxBlurHorizontal(dst, src, r);
            BoxBlurTotal(src, dst, r);
        }
        void BoxBlurHorizontal(NativeArray<RGBA64> src, NativeArray<RGBA64> dst, int r)
        {
            float iarr = 1f / (r + r + 1);
            for (int i = 0; i < h; i++)
            {
                ...
                {
                    val += (int4)src[ri++] - fv;
                    dst[ti++] = (RGBA64)(int4)math.round(val * iarr);
                }
        ...
        void BoxBlurTotal(NativeArray<RGBA64> src, NativeArray<RGBA64> dst, int r)
        {
