...
namespace RawTextureDataProcessingExamples
{
    ...
    public struct GaussianBlurRGB24Job : Unity.Jobs.IJob
    {
        ...
        void BoxBlur ( NativeArray<RGB24> src , NativeArray<RGB24> dst , int r )
        {
            BoxBlurHorizontal( dst , src , r );
            BoxBlurTotal( src , dst , r );
        }
        void BoxBlurHorizontal ( NativeArray<RGB24> src , NativeArray<RGB24> dst , int r )
        {
            float iarr = 1f / (r+r+1);
            for( int i=0 ; i<h ; i++ )
            {
        ...
        void BoxBlurTotal ( NativeArray<RGB24> src , NativeArray<RGB24> dst , int r )
        {
